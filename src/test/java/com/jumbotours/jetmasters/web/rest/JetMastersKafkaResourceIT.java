...
import com.jumbotours.jetmasters.config.KafkaProperties;
...
class JetMastersKafkaResourceIT {
    ...
    void setup() {
        ...
        JetMastersKafkaResource kafkaResource = new JetMastersKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/jet-masters-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/jet-masters-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
